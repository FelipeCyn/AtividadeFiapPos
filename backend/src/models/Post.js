...
class Post {
  static async create({ titulo, conteudo, autor, teacher_id }) {
    const result = await pool.query(
      'INSERT INTO posts (titulo, conteudo, autor, teacher_id) VALUES ($1, $2, $3, $4) RETURNING *',
      [titulo, conteudo, autor, teacher_id]
    );
  ...
  static async findByTeacherId(teacherId) {
    const result = await pool.query(
      'SELECT * FROM posts WHERE teacher_id = $1 ORDER BY created_at DESC',
      [teacherId]
    );
    return result.rows;
  }
